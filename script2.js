...
//rows stores the different pages or rows where each page or row will show maximum of data of 10 users
let rows=[];

//btns stores button element which will be used to go to a specific page or row
let btns=[];

//this variable is used to maintain the current size of each row or page also current size of each page or row cannot exceed 10
let currsize=0;

//this variable is used to maintain the current row or page
let cr=0;

//this variable is used to maintain the current button 
let cb=0;

//getting the container 1 which will show each page (one page at a time) from all the different pages
container1=document.getElementById("container1");

//creating a div element which will act as a row of container1
let row1=document.createElement('div');
row1.setAttribute("class","row justify-content-center");

//getting the elements where the prev button ,next button and all the buttons of different pages will be shown
let prev=document.getElementById('prev');
let buttons=document.getElementById('buttons');
let next=document.getElementById('next');

//a function which will set the attributes and content of each button
...
let setCol=(col,data)=>
{
    ...
    col.style.boxShadow="0px 5px 10px #FF00CC";
    col.style.border="5px solid #5bc0de"
...
//creating a button element which will act as previous button 
let prebtn=document.createElement('button');
setButton(prebtn,"prev");
prev.append(prebtn);

//creating a button element which will act as next button
let nextbtn=document.createElement('button');
setButton(nextbtn,"next");
next.append(nextbtn);

//showing only buttons of first 7 pages becuase only max of 7 buttons will be shown in the main page at a time
let first=0;
let last=6;
...
//function to update the contents of the main page dynamically after a button is clicked
let update=()=>{
    //showing the contents of current button which is clicked
    container1.innerHTML="";
    container1.append(rows[cb]);

    document.getElementById(`${cb+1}`).setAttribute('class','btn btn-outline-info active');

    //updating the page number which will be shown at the main page
    page=document.getElementById('page');
    page.innerHTML=`Page ${cb+1} of ${cr}`;

    //check if the prev button and next button should enabled or disabled
    prebtn.disabled=(cb-1<0)?true:false;
    nextbtn.disabled=(cb+1>=btns.length)?true:false;

    //logic to maintain 7 buttons at a time
    if(cb==first && cb-1>=0)
    {
...
//called when the page is loaded for the first time,here the current page will be 1 
update();

//adding eventlisteners to each buttons
